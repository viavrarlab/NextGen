...
public class InputManager : MonoBehaviour
{
    ...
    {
        // Trigger----------------------------------------------------------------------------
        if (ViveInput.GetPressDown(Hand, m_TriggerButton))
            OnTriggerDown.Invoke();

        if (ViveInput.GetPressUp(Hand, m_TriggerButton))
            OnTriggerUp.Invoke();

        if (ViveInput.GetPress(Hand, m_TriggerButton))
            OnTriggerHold.Invoke();

        float TriggerPullFloat = ViveInput.GetAxis(Hand, m_TriggerPull);
